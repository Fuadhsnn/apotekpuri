...
END;
    ...
    public function download($filename)
    {
        ...
        // Render view ke HTML
        $html = view('exports.penjualan-pdf', $viewData)->render();
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'UTF-8') !== false || 
                    ...
                    try {
                        ...
                        // Log error kedua
                        \Illuminate\Support\Facades\Log::error('Second attempt failed: ' . $e2->getMessage());
                        \Illuminate\Support\Facades\Log::error($e2->getTraceAsString());
                        
                        // Coba pendekatan terakhir dengan HTML yang sangat disederhanakan
                        try {
                            // Buat instance mPDF baru dengan konfigurasi minimal
                            $mpdf = new Mpdf([
                                'mode' => 'utf-8',
                                'format' => 'A4',
                                'default_font' => 'dejavusans',
                            ...
                            // Sederhanakan HTML secara ekstrem
                            $html = preg_replace('/<style[^>]*>.*?<\/style>/is', '', $html); // Hapus semua CSS
                            $html = preg_replace('/<script[^>]*>.*?<\/script>/is', '', $html); // Hapus semua JavaScript
                            $html = strip_tags($html, '<table><tr><td><th><p><div><span><br><b><i><strong><em><h1><h2><h3><h4><h5><h6>'); // Hanya izinkan tag dasar
                            ...
                            // Log error ketiga
                            \Illuminate\Support\Facades\Log::error('Third attempt failed: ' . $e3->getMessage());
                            \Illuminate\Support\Facades\Log::error($e3->getTraceAsString());
                            
                            // Tampilkan pesan error yang lebih informatif
                            abort(500, 'Error generating PDF after multiple attempts. Please contact administrator.');
                        }
                    }
                }
